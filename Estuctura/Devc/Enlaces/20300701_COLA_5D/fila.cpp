...
void lista::del(){
	nodo* p = NULL;
	if(i){	//si el primero es nulo solo no va a eliminar nada, si hay algo sigue
	a=i;	//auxiliar va a ser igual a nuestra i
	//p=i;
		if(a->sig == NULL){
			cout<< "Solo hay un valor que eliminar "<<endl;
			delall();
		}else{
		
			//cout << "eif"<<endl;		
			while(a){ 	//mientras a sea verdadero se repetira
			//	cout << "ewhile "<< a->sig << endl;
				if(a->sig ==NULL)	{ //si es nulo el siguiente significa que estamos en la ultima posicion
				//	cout << "f"<<endl;
					p=i;
						while(p->sig != a){ //para que tengamos una variable antes de nuestra posicon actual
							p = p->sig; //para dejar un espacio
						}
						p->sig = a->sig;
					delete a; //y eliminamos la variable ultima	
					break;
				}else{ //si el siguiente lugar no es nulo, entonces pasamos al siguiente
				//	cout << "sig"<<endl;
					a=a->sig; 
				}
			}
...
int main(){
	...
	lista ob1;
		do{
			cout<<"Fila: ultimo que entra, ultimo que se va"<<endl;
